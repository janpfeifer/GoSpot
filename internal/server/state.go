...
func (s *ServerState) HandleWS(w http.ResponseWriter, r *http.Request) {
	...
	// Ensure player ID
	if playerID == "" { // Should be generated by client, but fallback
		playerID = fmt.Sprintf("%d", time.Now().UnixNano())
	}
	klog.Infof("HandleWS: Player %s (%s) joining table %s", playerName, playerID, tableID)
	table, player := s.joinTable(tableID, playerName, playerID, conn)
...
func (s *ServerState) joinTable(tableID string, playerName, playerID string, conn *websocket.Conn) (*game.Table, *game.Player) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	klog.Infof("joinTable: Adding player %q to table %s", playerName, tableID)
